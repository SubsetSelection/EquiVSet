...
class ACNNPredictor(nn.Module):
    ...
    def __init__(self, in_size, hidden_sizes, weight_init_stddevs,
        ...
        self.fea_layer = MLP(num_layers=1, input_dim=1922, hidden_dim=2048, output_dim=256)
    ...
    def forward(self, batch_size, frag1_node_indices_in_complex, frag2_node_indices_in_complex,
        ...
        ligand_energy = ligand_feats.reshape(batch_size, -1)   # (B, O)
        protein_energy = protein_feats.reshape(batch_size, -1) # (B, O)
        ...
        complex_protein_energy = complex_feats[frag2_node_indices_in_complex].reshape(
            batch_size, -1)
        complex_energy = torch.cat([complex_ligand_energy, complex_protein_energy], dim=-1)
        
        fea = torch.cat([ligand_energy, protein_energy, complex_energy], dim=-1)
        return self.fea_layer(fea)
